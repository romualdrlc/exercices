import { findGameFromSlug, Game } from "../src/game-finder";

it("Should find a game from its slug", () => {
  expect.assertions(2);

  const ff7r = findGameFromSlug(games, "final-fantasy-vii-remake");
  ...
  const redRonin = findGameFromSlug(games, "red-ronin");

  expect(redRonin).toEqual({
    name: "Red Ronin",
    slug: "red-ronin",
...
it("Should return undefined if the game does not exist", () => {
  ...
  const result = findGameFromSlug(games, "plop");
